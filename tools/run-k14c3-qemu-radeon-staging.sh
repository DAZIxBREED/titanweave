...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C3_SERIAL_LOG:-$ROOT/build/k14c3-serial.log}"
NVME_IMAGE="${K14C3_NVME_IMAGE:-$ROOT/build/k14c3-nvme-test.img}"
NVME_SIZE="${K14C3_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C3.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c3nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c3nvme,serial=TWK14C3NVME001
    ...
    -device edu,id=twk14c3iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C3 Radeon bare-metal staging QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C3 therefore verifies the Radeon
staging contract remains fail-closed: no vendor MMIO writes, firmware upload,
command submission, or Radeon bus mastering may be promoted by the surrogate.
...
checker_status=0
"$ROOT/tools/check-k14c3-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
