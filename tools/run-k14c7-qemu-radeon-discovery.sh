...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C7_SERIAL_LOG:-$ROOT/build/k14c7-serial.log}"
NVME_IMAGE="${K14C7_NVME_IMAGE:-$ROOT/build/k14c7-nvme-test.img}"
NVME_SIZE="${K14C7_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C7.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c7nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c7nvme,serial=TWK14C7NVME001
    ...
    -device edu,id=twk14c7iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C7 Radeon MMIO/firmware discovery staging QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C7 verifies the post-C6 Radeon discovery contract without promoting hardware:
no physical Radeon means no Radeon BAR mapping, register reads/writes, VBIOS access, firmware upload,
...
checker_status=0
"$ROOT/tools/check-k14c7-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
