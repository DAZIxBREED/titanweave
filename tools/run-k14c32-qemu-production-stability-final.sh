...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C32_SERIAL_LOG:-$ROOT/build/k14c32-serial.log}"
NVME_IMAGE="${K14C32_NVME_IMAGE:-$ROOT/build/k14c32-nvme-test.img}"
NVME_SIZE="${K14C32_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C32.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c31nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c31nvme,serial=TWK14C32NVME001
    ...
    -device edu,id=twk14c32iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C32 production-stability-final QEMU test
  serial log     : $LOG
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C32 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c32-serial-log.sh" "$LOG" || checker_status=$?
