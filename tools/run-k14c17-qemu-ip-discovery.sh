...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C17_SERIAL_LOG:-$ROOT/build/k14c17-serial.log}"
NVME_IMAGE="${K14C17_NVME_IMAGE:-$ROOT/build/k14c17-nvme-test.img}"
NVME_SIZE="${K14C17_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C17.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c17nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c17nvme,serial=TWK14C17NVME001
    ...
    -device edu,id=twk14c17iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C17 IP-discovery QEMU test
  serial log     : $LOG
...
checker_status=0
"$ROOT/tools/check-k14c17-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
