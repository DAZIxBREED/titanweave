...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C18_SERIAL_LOG:-$ROOT/build/k14c18-serial.log}"
NVME_IMAGE="${K14C18_NVME_IMAGE:-$ROOT/build/k14c18-nvme-test.img}"
NVME_SIZE="${K14C18_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C18.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c18nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c18nvme,serial=TWK14C18NVME001
    ...
    -device edu,id=twk14c18iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C18 snapshot-verification QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C17 provides the bounded AMD IP-discovery parser. K14.C18 adds the source-backed checksum verifier and discovery-TMR acquisition contract. QEMU has no physical Radeon, so live TMR/VRAM snapshot acquisition is intentionally deferred. The synthetic snapshot must pass both the binary checksum and IP_DISCOVERY table checksum. MMIO writes, firmware upload, command submission, and Radeon bus-master enable remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
...
checker_status=0
"$ROOT/tools/check-k14c18-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
