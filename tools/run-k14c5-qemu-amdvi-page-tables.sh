...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C5_SERIAL_LOG:-$ROOT/build/k14c5-serial.log}"
NVME_IMAGE="${K14C5_NVME_IMAGE:-$ROOT/build/k14c5-nvme-test.img}"
NVME_SIZE="${K14C5_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C5.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c5nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c5nvme,serial=TWK14C5NVME001
    ...
    -device edu,id=twk14c5iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C5 AMD-Vi page-table engine QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C5 therefore verifies the AMD-Vi page-table engine contract remains fail-closed:
QEMU has no physical Radeon, so no Radeon AMD-Vi device/page-table image, MMIO write, firmware upload,
...
checker_status=0
"$ROOT/tools/check-k14c5-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
