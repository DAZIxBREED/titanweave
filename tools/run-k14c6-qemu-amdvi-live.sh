...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C6_SERIAL_LOG:-$ROOT/build/k14c6-serial.log}"
NVME_IMAGE="${K14C6_NVME_IMAGE:-$ROOT/build/k14c6-nvme-test.img}"
NVME_SIZE="${K14C6_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C6.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c6nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c6nvme,serial=TWK14C6NVME001
    ...
    -device edu,id=twk14c6iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C6 live AMD-Vi hardware-programming boundary QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C6 verifies that Intel VT-d/QEMU can never arm the physical AMD-Vi programming path:
QEMU has no physical Radeon, so no AMD-Vi physical register programming, Radeon MMIO write, firmware upload,
command submission, or Radeon bus mastering may be promoted by any surrogate.
...
checker_status=0
"$ROOT/tools/check-k14c6-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
