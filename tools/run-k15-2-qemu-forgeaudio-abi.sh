...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K15_2_SERIAL_LOG:-$ROOT/build/k15-2-serial.log}"
NVME_IMAGE="${K15_2_NVME_IMAGE:-$ROOT/build/k15-2-nvme-test.img}"
NVME_SIZE="${K15_2_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K15_2.fd"
...
cat <<MSG
Titanweave K15.2 ForgeAudio kernel ABI QEMU test
  serial log     : $LOG
...
K15.2 retains the frozen/qualified K15.1 and K14.C32 paths while adding the versioned ForgeAudio kernel/userspace object ABI gate.
...
QEMU does not emulate a native Radeon. K14.C32 remains the frozen Radeon production/stability baseline. K15.1 still runs its real 1 kHz scheduler qualification. K15.2 then initializes ForgeAudio ABI v1 and qualifies real bounded buffer/clock/event/fence objects plus strict stream lifecycle semantics. QEMU intentionally registers zero audio devices; fake hardware is forbidden and the HDA backend remains K15.4.
...
TEST_TIMEOUT="${K15_2_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K15.2 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k15-2-serial-log.sh" "$LOG" || checker_status=$?
