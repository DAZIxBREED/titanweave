...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C16_SERIAL_LOG:-$ROOT/build/k14c16-serial.log}"
NVME_IMAGE="${K14C16_NVME_IMAGE:-$ROOT/build/k14c16-nvme-test.img}"
NVME_SIZE="${K14C16_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C16.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c16nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c16nvme,serial=TWK14C16NVME001
    ...
    -device edu,id=twk14c16iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C16 reviewed-MMIO-write QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C16 verifies the reviewed Radeon MMIO-write transaction gate. QEMU has no physical Radeon, so the MMIO identity-write is intentionally deferred. Live execution additionally requires an exact source-reviewed register index plus a trusted per-IP base; Titanweave never guesses either. Firmware upload, command submission, and Radeon bus-master enable remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
...
checker_status=0
"$ROOT/tools/check-k14c16-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
