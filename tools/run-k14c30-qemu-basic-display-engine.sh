...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C30_SERIAL_LOG:-$ROOT/build/k14c30-serial.log}"
NVME_IMAGE="${K14C30_NVME_IMAGE:-$ROOT/build/k14c30-nvme-test.img}"
NVME_SIZE="${K14C30_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C30.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c30nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c30nvme,serial=TWK14C30NVME001
    ...
    -device edu,id=twk14c30iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C30 complete-basic-display-engine QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C30 is the locked-roadmap complete basic display engine milestone. It consumes frozen C29 and runs an operational GOP-backed display engine: EDID parse/mode-selection logic, bounded connector/CRTC/plane ownership, two real GTT scanout surfaces, live volatile framebuffer presents with page-flip verification, atomic current-mode commit/rollback, and hotplug bookkeeping. DCN401 resource counts are source-reviewed. QEMU has no Radeon, so native DCN register programming and physical HPD are explicitly not claimed. The firmware GOP scanout is the real active backend.
The kernel intentionally halts after C30 native-service qualification.
MSG

TEST_TIMEOUT="${K14C30_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C30 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c30-serial-log.sh" "$LOG" || checker_status=$?
