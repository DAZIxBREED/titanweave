...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C1_SERIAL_LOG:-$ROOT/build/k14c1-serial.log}"
NVME_IMAGE="${K14C1_NVME_IMAGE:-$ROOT/build/k14c1-nvme-test.img}"
NVME_SIZE="${K14C1_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C1.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c1nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c1nvme,serial=TWK14C1NVME001
    ...
    -device edu,id=twk14c1iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C1 native GPU ownership QEMU test
  serial log     : $LOG
...
QEMU does not emulate an AMD/Intel/NVIDIA adapter, so K14.C1 must leave native
activation fenced while proving the AMD-first ownership and VRAM/GTT contracts.
...
checker_status=0
"$ROOT/tools/check-k14c1-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
