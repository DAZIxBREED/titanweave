...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C23_SERIAL_LOG:-$ROOT/build/k14c23-serial.log}"
NVME_IMAGE="${K14C23_NVME_IMAGE:-$ROOT/build/k14c23-nvme-test.img}"
NVME_SIZE="${K14C23_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C23.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c23nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c23nvme,serial=TWK14C23NVME001
    ...
    -device edu,id=twk14c23iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C23 dual-probe-stability QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C23 consumes frozen C22's exact restore-verified GFX12 SCRATCH_REG0 transaction and does not widen the writable register set. On supported Navi48 bare metal C23 first requires the value restored by C22 to persist, then derives two distinct one-bit probe values internally and executes two bounded probe/readback/restore cycles on the same checksum-backed SCRATCH_REG0 target. Exact restoration is mandatory after each cycle, with one final restore retry per cycle allowed only for recovery. Caller-supplied addresses/values, arbitrary MMIO writes, MM_INDEX fallback, firmware upload, command submission, BAR resizing, and Radeon bus-master enable remain forbidden. On QEMU the physical transaction is explicitly deferred while source/self-tests, ABI, userspace reporting, and fallback behavior qualify. VirtIO-GPU/GOP remain the qualified fallback.
The kernel intentionally halts after native-service qualification.
MSG

TEST_TIMEOUT="${K14C23_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C23 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c23-serial-log.sh" "$LOG" || checker_status=$?
