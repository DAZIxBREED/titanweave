...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C21_SERIAL_LOG:-$ROOT/build/k14c21-serial.log}"
NVME_IMAGE="${K14C21_NVME_IMAGE:-$ROOT/build/k14c21-nvme-test.img}"
NVME_SIZE="${K14C21_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C21.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c21nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c21nvme,serial=TWK14C21NVME001
    ...
    -device edu,id=twk14c21iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C21 reviewed-MMIO-rebind QEMU test
  serial log     : $LOG
