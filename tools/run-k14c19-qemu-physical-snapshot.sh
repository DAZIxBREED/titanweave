...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C19_SERIAL_LOG:-$ROOT/build/k14c19-serial.log}"
NVME_IMAGE="${K14C19_NVME_IMAGE:-$ROOT/build/k14c19-nvme-test.img}"
NVME_SIZE="${K14C19_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C19.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c19nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c19nvme,serial=TWK14C19NVME001
    ...
    -device edu,id=twk14c19iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C19 physical-snapshot QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C19 adds the first physical discovery-snapshot acquisition path: source-backed DRIVER_SCRATCH TMR location reads, read-only PCIe ECAM inspection of the current BAR0 Resizable-BAR size, and a direct CPU read-only BAR0 snapshot only when the complete TMR is already visible. The MM_INDEX/MM_DATA fallback remains disabled because it requires MMIO writes. QEMU must therefore take the explicit no-Radeon deferred path while the C19 policy/self-tests and userspace ABI qualify. MMIO writes, firmware upload, command submission, and Radeon bus-master enable remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
...
checker_status=0
"$ROOT/tools/check-k14c19-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
