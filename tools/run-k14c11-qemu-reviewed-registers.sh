...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C11_SERIAL_LOG:-$ROOT/build/k14c11-serial.log}"
NVME_IMAGE="${K14C11_NVME_IMAGE:-$ROOT/build/k14c11-nvme-test.img}"
NVME_SIZE="${K14C11_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C11.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c11nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c11nvme,serial=TWK14C11NVME001
    ...
    -device edu,id=twk14c11iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C11 reviewed-register/IP-base QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C11 verifies the post-C10 reviewed-register/IP-base contract without promoting hardware:
no physical Radeon means no register dereference; on real hardware a reviewed profile still requires a trusted per-IP base map before any MMIO read. Register writes, firmware upload, command submission, and Radeon bus mastering remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
...
checker_status=0
"$ROOT/tools/check-k14c11-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
