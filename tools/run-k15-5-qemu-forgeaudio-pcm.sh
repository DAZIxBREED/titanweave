...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K15_5_SERIAL_LOG:-$ROOT/build/k15-5-serial.log}"
NVME_IMAGE="${K15_5_NVME_IMAGE:-$ROOT/build/k15-5-nvme-test.img}"
NVME_SIZE="${K15_5_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K15_5.fd"
...
cat <<MSG
Titanweave K15.5 ForgeAudio PCM format engine QEMU qualification
  serial log       : $LOG
...
K15.5 consumes frozen/qualified K15.4. Frozen K15.4 must first re-qualify the real HDA hardware model. K15.5 then
runs the allocation-free PCM format engine against canonical rate/width vectors
and the actual HDA playback/capture endpoints registered by K15.4. It must prove
interleaved/planar byte preservation, named channel mapping without hidden
mixing, exact/nearest supported-rate negotiation, HDA stream-format roundtrips,
K15.3-bounded DMA geometry, and fail-closed unsupported requests. No fake audio
device or later-gate resampler/mixer/server work can pass this gate.

K15.6 ForgeAudioD remains locked until the K15.5 runtime gate passes.
MSG

TEST_TIMEOUT="${K15_5_TIMEOUT:-150}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K15.5 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k15-5-serial-log.sh" "$LOG" || checker_status=$?
