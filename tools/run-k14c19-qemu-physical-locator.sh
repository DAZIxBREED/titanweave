...
cat <<MSG
Titanweave K14.C19 physical-locator QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C18 provides the checksum-qualified AMD discovery snapshot contract. K14.C19 opens only the bounded read-only physical locator path: RCC_CONFIG_MEMSIZE and DRIVER_SCRATCH_0..2 through BAR5. QEMU has no physical Radeon, so those four locator reads are intentionally deferred. Discovery payload reads, MMIO writes, firmware upload, command submission, and Radeon bus-master enable remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
