...
checks={
 ...
 'kernel/weavecore/src/service.rs':['ARCHIVE.ELF','DRIVERD.ELF','DISPLAYD.ELF','AUDIOD.ELF','ServiceRole::Display','ServiceRole::Audio','ServiceRole::AudioClient'],
...
for app in ['init','logd','console','displayd','archive','trustd','driverd','forgeaudiod','audioclient','shell']:
...
assert (root/'K14C29_RUNTIME_QUALIFICATION.md').is_file()
...
assert (root/'K14C31_RUNTIME_QUALIFICATION.md').is_file()
assert (root/'K14C32_IMPLEMENTATION.md').is_file()
assert (root/'K14C32_TESTER_GUIDE.md').is_file()
assert (root/'K14C32_SOURCE_STATUS.md').is_file()
assert (root/'K14C32_RUNTIME_QUALIFICATION.md').is_file()
...
if command -v clang >/dev/null 2>&1 && command -v ld.lld >/dev/null 2>&1; then
  ...
  for app in INIT LOGD CONSOL DISPLAYD ARCHIVE TRUSTD DRIVERD AUDIOD AUDIOCLT SHELL; do
...
python3 "$ROOT/tools/test-k14c32-production-stability-final.py"
python3 "$ROOT/tools/test-k15-1-forgeaudio-rt.py"
python3 "$ROOT/tools/test-k15-2-forgeaudio-abi.py"
