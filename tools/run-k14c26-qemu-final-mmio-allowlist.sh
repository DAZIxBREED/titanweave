...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C26_SERIAL_LOG:-$ROOT/build/k14c26-serial.log}"
NVME_IMAGE="${K14C26_NVME_IMAGE:-$ROOT/build/k14c26-nvme-test.img}"
NVME_SIZE="${K14C26_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C26.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c26nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c26nvme,serial=TWK14C26NVME001
    ...
    -device edu,id=twk14c26iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C26 final-k14-mmio-allowlist QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C26 is the final K14 completion gate. It consumes frozen C25 and adds no new write transaction. On supported Navi48 bare metal it re-resolves the exact checksum-backed GFX12 GC base-slot proof, resolves AMD-generated SCRATCH_REG1 at dword 0x2041 / BASE_IDX 1, proves it is distinct and exactly one dword after the frozen SCRATCH_REG0 target, materializes the final two-entry reviewed MMIO allowlist, and performs four bounded volatile reads from REG1 while performing zero C26 MMIO writes. Caller-supplied addresses/values, REG1 writes, arbitrary MMIO, MM_INDEX fallback, firmware upload, command submission, BAR resizing, and Radeon bus-master enable remain forbidden. On QEMU the physical REG1 read is explicitly deferred while source/self-tests, ABI, userspace reporting, final K14 policy, and fallback behavior qualify. VirtIO-GPU/GOP remain the qualified fallback.
The kernel intentionally halts after native-service qualification.
MSG

TEST_TIMEOUT="${K14C26_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C26 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c26-serial-log.sh" "$LOG" || checker_status=$?
