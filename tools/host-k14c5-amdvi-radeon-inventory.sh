#!/usr/bin/env bash
set -euo pipefail
echo 'Titanweave K14.C5 host AMD-Vi/Radeon inventory (read-only)'
echo
...
if [[ -r /sys/firmware/acpi/tables/IVRS ]]; then
  echo "=== IVRS table ==="
  ls -l /sys/firmware/acpi/tables/IVRS
  sha256sum /sys/firmware/acpi/tables/IVRS 2>/dev/null || true
fi
