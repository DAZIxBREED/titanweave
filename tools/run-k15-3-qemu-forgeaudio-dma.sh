...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K15_3_SERIAL_LOG:-$ROOT/build/k15-3-serial.log}"
NVME_IMAGE="${K15_3_NVME_IMAGE:-$ROOT/build/k15-3-nvme-test.img}"
NVME_SIZE="${K15_3_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K15_3.fd"
...
cat <<MSG
Titanweave K15.3 ForgeAudio audio DMA transport QEMU test
  serial log     : $LOG
...
K15.3 retains frozen K15.1/K15.2 and K14.C32 while adding the backend-neutral ForgeAudio cyclic DMA transport gate.
...
QEMU does not emulate a native Radeon. K14.C32 remains frozen. K15.1 and K15.2 requalify first. K14.B then performs the real VT-d/EDU hardware translation proof. K15.3 allocates real contiguous audio-DMA memory, maps it through Titanweave's kernel DMA aperture, qualifies cyclic period ownership/position/XRUN accounting, and verifies production hardware access remains fail-closed without an exact translated audio requester lease. No HDA device or IRQ is invented; HDA hardware remains K15.4.

K14.C32 is the locked final Radeon production/stability milestone. It consumes frozen C31 and stress-tests real Titanweave GTT allocations, queue wrap/retirement, deliberate stuck-queue recovery, software IRQ/recovery lifecycles, interleaved compute/graphics work, repeated C30 scanout presentation, telemetry, power policy, the frozen userspace GPU ABI, shader precache state, and PCI multi-GPU inventory. QEMU has no Radeon, so physical-stress qualification remains explicitly false and is checked separately by the bare-metal evidence checker.
The kernel intentionally halts after K14.C32 userspace qualification.
MSG

TEST_TIMEOUT="${K15_3_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K15.3 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k15-3-serial-log.sh" "$LOG" || checker_status=$?
