...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K13C_SERIAL_LOG:-$ROOT/build/k13c-serial.log}"
NVME_IMAGE="${K13C_NVME_IMAGE:-$ROOT/build/k13c-nvme-test.img}"
NVME_SIZE="${K13C_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K13C.fd"
...
QEMU_ARGS=(
    ...
    -device nvme,drive=k13nvme,serial=TWK13CNVME001
...
cat <<MSG
Titanweave K13.C buffered compositor presentation QEMU test
  serial log     : $LOG
...
K13.C must retain the K12 stdvga/GOP fallback while presenting through the qualified K13.B VirtIO-GPU transport.
K13.C uses triple-buffered GPU resources, dirty-region transfer/flush, and device-echoed fences. ACCESS_PLATFORM remains disabled until hardware IOMMU page-table programming is implemented.
...
"$ROOT/tools/check-k13c-serial-log.sh" "$LOG" || true
exit "$status"
