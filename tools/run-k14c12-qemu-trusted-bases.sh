...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C12_SERIAL_LOG:-$ROOT/build/k14c12-serial.log}"
NVME_IMAGE="${K14C12_NVME_IMAGE:-$ROOT/build/k14c12-nvme-test.img}"
NVME_SIZE="${K14C12_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C12.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c12nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c12nvme,serial=TWK14C12NVME001
    ...
    -device edu,id=twk14c12iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C12 trusted-IP-base/live-read QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C12 verifies the post-C11 trusted-IP-base/live-read contract without promoting hardware:
...
checker_status=0
"$ROOT/tools/check-k14c12-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
