...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C13_SERIAL_LOG:-$ROOT/build/k14c13-serial.log}"
NVME_IMAGE="${K14C13_NVME_IMAGE:-$ROOT/build/k14c13-nvme-test.img}"
NVME_SIZE="${K14C13_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C13.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c13nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c13nvme,serial=TWK14C13NVME001
    ...
    -device edu,id=twk14c13iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C13 physical-read-proof QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C13 verifies the physical read-proof qualification contract without promoting hardware:
no physical Radeon means bare-metal evidence is intentionally deferred. On real hardware C13 requires inherited C12 status reads, sanity-checks the values, rechecks that bus mastering is off, and emits a compact proof fingerprint. Register writes, firmware upload, command submission, and Radeon bus mastering remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
...
checker_status=0
"$ROOT/tools/check-k14c13-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
