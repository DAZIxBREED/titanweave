...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C15_SERIAL_LOG:-$ROOT/build/k14c15-serial.log}"
NVME_IMAGE="${K14C15_NVME_IMAGE:-$ROOT/build/k14c15-nvme-test.img}"
NVME_SIZE="${K14C15_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C15.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c15nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c15nvme,serial=TWK14C15NVME001
    ...
    -device edu,id=twk14c15iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C15 controlled-write QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C15 verifies the first controlled write-transaction contract. QEMU has no physical Radeon, so the identity-write transaction is intentionally deferred. On real hardware C15 requires frozen C14 readiness, then performs only a width-correct 16-bit PCI Command identity write, readback, and bounded rollback. Radeon MMIO writes, firmware upload, command submission, and bus-master enable remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
...
checker_status=0
"$ROOT/tools/check-k14c15-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
