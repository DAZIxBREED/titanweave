...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C4_SERIAL_LOG:-$ROOT/build/k14c4-serial.log}"
NVME_IMAGE="${K14C4_NVME_IMAGE:-$ROOT/build/k14c4-nvme-test.img}"
NVME_SIZE="${K14C4_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C4.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c4nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c4nvme,serial=TWK14C4NVME001
    ...
    -device edu,id=twk14c4iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C4 Radeon exact-requester/AMD-Vi gate QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C4 therefore verifies the exact-requester/AMD-Vi contract remains fail-closed:
QEMU has no physical Radeon, so no exact Radeon domain, MMIO write, firmware upload,
...
checker_status=0
"$ROOT/tools/check-k14c4-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
