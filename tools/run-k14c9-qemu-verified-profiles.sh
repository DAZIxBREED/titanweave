...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C9_SERIAL_LOG:-$ROOT/build/k14c9-serial.log}"
NVME_IMAGE="${K14C9_NVME_IMAGE:-$ROOT/build/k14c9-nvme-test.img}"
NVME_SIZE="${K14C9_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C9.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c9nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c9nvme,serial=TWK14C9NVME001
    ...
    -device edu,id=twk14c9iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C9 verified Radeon profile/live safe-read QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C9 verifies the post-C8 profile/live safe-read contract without promoting hardware:
no physical Radeon means no live Radeon identity/MMIO access, register writes, firmware upload,
...
checker_status=0
"$ROOT/tools/check-k14c9-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
