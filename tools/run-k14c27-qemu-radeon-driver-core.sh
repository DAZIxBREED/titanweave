...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C27_SERIAL_LOG:-$ROOT/build/k14c27-serial.log}"
NVME_IMAGE="${K14C27_NVME_IMAGE:-$ROOT/build/k14c27-nvme-test.img}"
NVME_SIZE="${K14C27_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C27.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c27nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c27nvme,serial=TWK14C27NVME001
    ...
    -device edu,id=twk14c27iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C27 complete-radeon-driver-core QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C27 is the complete operational Radeon driver-core milestone. It consumes frozen C26 without opening new register or write authority. C27 resolves exact ForgeBus ownership, captures live PCI/BAR/VRAM/IOMMU topology on physical hardware, provides a permanent identity-based reviewed-MMIO read service, executes lifecycle/error/reset coordination, and installs a real interrupt-router route and handler. Physical interrupt delivery remains masked until C29; firmware upload, DMA/bus mastering and GPU command submission remain forbidden. In QEMU the software driver core, policy enforcement, interrupt handler dispatch self-test, ABI/userspace reporting and fallback behavior qualify while physical Radeon ownership/MMIO reads are explicitly deferred. VirtIO-GPU/GOP remain the qualified fallback.
The kernel intentionally halts after native-service qualification.
MSG

TEST_TIMEOUT="${K14C27_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C27 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c27-serial-log.sh" "$LOG" || checker_status=$?
