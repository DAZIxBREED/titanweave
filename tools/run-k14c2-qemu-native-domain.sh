...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C2_SERIAL_LOG:-$ROOT/build/k14c2-serial.log}"
NVME_IMAGE="${K14C2_NVME_IMAGE:-$ROOT/build/k14c2-nvme-test.img}"
NVME_SIZE="${K14C2_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C2.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c2nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c2nvme,serial=TWK14C2NVME001
    ...
    -device edu,id=twk14c2iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C2 persistent native-domain/AMD bring-up QEMU test
  serial log     : $LOG
...
QEMU does not emulate an AMD/Intel/NVIDIA adapter. K14.C2 therefore reuses EDU as a
real VT-d requester to retain one translated domain across three DMA epochs, then
revokes it. The surrogate must never promote the physical Radeon-domain bit.
...
checker_status=0
"$ROOT/tools/check-k14c2-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
