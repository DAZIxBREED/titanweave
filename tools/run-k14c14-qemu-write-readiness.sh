...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C14_SERIAL_LOG:-$ROOT/build/k14c14-serial.log}"
NVME_IMAGE="${K14C14_NVME_IMAGE:-$ROOT/build/k14c14-nvme-test.img}"
NVME_SIZE="${K14C14_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C14.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c14nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c14nvme,serial=TWK14C14NVME001
    ...
    -device edu,id=twk14c14iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C14 write-promotion-readiness QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C14 verifies the controlled write-promotion readiness contract without promoting hardware:
no physical Radeon means write-side eligibility is intentionally deferred. On real hardware C14 requires the frozen C13 physical proof, translated-domain continuity, trusted IP bases, BAR5 readiness, a nonzero proof fingerprint, and a fresh bus-master-off check. Register writes, firmware upload, command submission, and Radeon bus mastering remain fenced. VirtIO-GPU/GOP remain the qualified fallback.
...
checker_status=0
"$ROOT/tools/check-k14c14-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
