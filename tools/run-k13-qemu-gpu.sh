...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K13_SERIAL_LOG:-$ROOT/build/k13-serial.log}"
NVME_IMAGE="${K13_NVME_IMAGE:-$ROOT/build/k13-nvme-test.img}"
NVME_SIZE="${K13_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K13.fd"
...
QEMU_ARGS=(
    ...
    -device nvme,drive=k13nvme,serial=TWK13NVME001
    -device qemu-xhci,id=xhci
    -device usb-kbd,bus=xhci.0
    -device usb-tablet,bus=xhci.0
    -vga std
    -device virtio-gpu-pci,id=twk13gpu,max_outputs=1
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K13.A GPU-foundation QEMU test
  serial log     : $LOG
  display        : $DISPLAY_BACKEND
  K12 scanout    : stdvga/GOP fallback retained
  GPU candidate  : VirtIO-GPU PCI (probe-only in K13.A)
  xHCI/HID       : enabled
  VT-d           : ${K13_IOMMU:-1}

K13.A must discover the VirtIO-GPU without enabling it from the new GPU probe.
...
"$ROOT/tools/check-k13-serial-log.sh" "$LOG" || true
exit "$status"
