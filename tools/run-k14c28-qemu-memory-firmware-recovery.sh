...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C28_SERIAL_LOG:-$ROOT/build/k14c28-serial.log}"
NVME_IMAGE="${K14C28_NVME_IMAGE:-$ROOT/build/k14c28-nvme-test.img}"
NVME_SIZE="${K14C28_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C28.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c28nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c28nvme,serial=TWK14C28NVME001
    ...
    -device edu,id=twk14c28iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C28 memory-firmware-recovery QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C28 is the locked-roadmap memory + firmware + recovery milestone. It consumes frozen C27 and implements real reclaimable GTT/system backing, cacheable supervisor DMA mappings, BAR0 VRAM reservation/reclamation, AMD common-firmware header parsing with CRC32 and SHA-256 staging verification, pinned firmware ownership, and an executable watchdog/resource-safe software recovery path. C28 does not install GPU page tables, enable Radeon bus mastering/DMA, submit commands, enable physical Radeon interrupts, upload firmware into silicon, or claim a physical ASIC reset; C29 owns the execution/DMA transition. In QEMU the real GTT allocation/map/readback/unmap/reclaim path, firmware parser/CRC logic, watchdog/recovery logic, ABI/userspace reporting and explicit hardware-deferred path qualify. VirtIO-GPU/GOP remain the qualified fallback.
The kernel intentionally halts after native-service qualification.
MSG

TEST_TIMEOUT="${K14C28_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C28 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c28-serial-log.sh" "$LOG" || checker_status=$?
