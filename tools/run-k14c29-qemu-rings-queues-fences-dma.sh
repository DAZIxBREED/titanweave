...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C29_SERIAL_LOG:-$ROOT/build/k14c29-serial.log}"
NVME_IMAGE="${K14C29_NVME_IMAGE:-$ROOT/build/k14c29-nvme-test.img}"
NVME_SIZE="${K14C29_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C29.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c29nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c29nvme,serial=TWK14C29NVME001
    ...
    -device edu,id=twk14c29iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C29 rings-queues-fences-dma QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C29 is the locked-roadmap rings + queues + fences + DMA milestone. It consumes frozen C28 and executes a real GTT-backed SDMA ring, FIFO submission queue, timeline fence, typed SDMA COPY/FENCE packet codec, and an owned-memory copy/fence executor over actual Titanweave mappings. The source-reviewed GFX12 SDMA0 queue-0 register plan is also validated. Physical SDMA programming and Radeon bus mastering remain fail-closed because QEMU has no Radeon and C28 intentionally did not claim firmware-in-silicon, GPU page tables, or a persistent translated Radeon IOMMU domain. Raw packet submission and caller-selected MMIO are forbidden. VirtIO-GPU/GOP remain the qualified fallback.
The kernel intentionally halts after native-service qualification.
MSG

TEST_TIMEOUT="${K14C29_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C29 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c29-serial-log.sh" "$LOG" || checker_status=$?
