...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C31_SERIAL_LOG:-$ROOT/build/k14c31-serial.log}"
NVME_IMAGE="${K14C31_NVME_IMAGE:-$ROOT/build/k14c31-nvme-test.img}"
NVME_SIZE="${K14C31_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C31.fd"
...
QEMU_ARGS=(
    ...
    -device nvme,drive=k14c31nvme,serial=TWK14C31NVME001
    ...
    -device edu,id=twk14c31iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C31 graphics-compute-execution QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C31 is the locked-roadmap graphics+compute execution milestone. It consumes frozen C30 and executes real Titanweave-owned GTT shader objects, typed command buffers, separate compute/graphics queues, timeline fences, a numerically verified vector-add dispatch, and a triangle draw into C30 scanout followed by a live GOP framebuffer present. QEMU has no Radeon, so this is explicitly the Titanweave reference execution backend; physical Radeon CP/GFX queue programming and native AMD ISA execution are not claimed.
The kernel intentionally halts after C30 native-service qualification.
MSG

TEST_TIMEOUT="${K14C31_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C31 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c31-serial-log.sh" "$LOG" || checker_status=$?
