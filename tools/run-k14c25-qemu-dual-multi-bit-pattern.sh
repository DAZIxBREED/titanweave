...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C25_SERIAL_LOG:-$ROOT/build/k14c25-serial.log}"
NVME_IMAGE="${K14C25_NVME_IMAGE:-$ROOT/build/k14c25-nvme-test.img}"
NVME_SIZE="${K14C25_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C25.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c25nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c25nvme,serial=TWK14C25NVME001
    ...
    -device edu,id=twk14c25iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C25 dual-multi-bit-pattern QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C25 consumes frozen C24 reversible four-bit-pattern proof and does not widen the writable register set. On supported Navi48 bare metal C25 first requires the value restored after C24 to persist, then derives two distinct deterministic four-bit patterns internally. Cycle A writes/reads/restores one four-bit pattern, an explicit inter-cycle read must still equal the original value, and cycle B writes/reads/restores a second distinct four-bit pattern. Each cycle permits one final restore retry only for recovery. The target remains the exact checksum-backed GFX12 SCRATCH_REG0 used by C21-C24. Caller-supplied addresses/values, arbitrary MMIO writes, MM_INDEX fallback, firmware upload, command submission, BAR resizing, and Radeon bus-master enable remain forbidden. On QEMU the physical transaction is explicitly deferred while source/self-tests, ABI, userspace reporting, and fallback behavior qualify. VirtIO-GPU/GOP remain the qualified fallback.
The kernel intentionally halts after native-service qualification.
MSG

TEST_TIMEOUT="${K14C25_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K14.C25 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k14c25-serial-log.sh" "$LOG" || checker_status=$?
