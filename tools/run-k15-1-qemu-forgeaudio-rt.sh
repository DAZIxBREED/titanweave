...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K15_1_SERIAL_LOG:-$ROOT/build/k15-1-serial.log}"
NVME_IMAGE="${K15_1_NVME_IMAGE:-$ROOT/build/k15-1-nvme-test.img}"
NVME_SIZE="${K15_1_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K15_1.fd"
...
cat <<MSG
Titanweave K15.1 ForgeAudio real-time execution QEMU test
  serial log     : $LOG
...
K15.1 retains the frozen K14.C32 path and all earlier qualification while adding the ForgeAudio real-time scheduler gate.
...
QEMU does not emulate a native Radeon. K14.C32 remains the frozen Radeon production/stability baseline. K15.1 runs a real 1 kHz scheduler qualification before the inherited graphics/device path: fixed-priority RT dispatch, hard CPU reservation/affinity, period/deadline/budget accounting, a priority-inheriting blocking mutex, bounded preemption guard, eight periodic audio jobs and a competing normal workload.
...
TEST_TIMEOUT="${K15_1_TIMEOUT:-120}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K15.1 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k15-1-serial-log.sh" "$LOG" || checker_status=$?
