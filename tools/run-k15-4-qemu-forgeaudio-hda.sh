...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K15_4_SERIAL_LOG:-$ROOT/build/k15-4-serial.log}"
NVME_IMAGE="${K15_4_NVME_IMAGE:-$ROOT/build/k15-4-nvme-test.img}"
NVME_SIZE="${K15_4_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K15_4.fd"
...
cat <<MSG
Titanweave K15.4 ForgeAudio real HDA hardware backend QEMU qualification
  serial log       : $LOG
...
K15.4 consumes frozen K15.3. The controller must be discovered as a real PCI
HDA function, reset through MMIO, run real CORB/RIRB DMA, enumerate its codec
and widgets, map CORB/RIRB/BDLs/audio rings into the exact HDA requester VT-d
domain, deliver real stream completion MSI through Titanweave's interrupt
router, complete two playback and two capture periods, prove capture DMA changed
memory, revoke bus mastering/translation, then register one HDA device and two
ForgeAudio endpoints. No fake DMA, fake IRQ, or placeholder device can pass.

K15.5 remains locked until this runtime gate passes.
MSG

TEST_TIMEOUT="${K15_4_TIMEOUT:-150}"
...
if (( halt_seen )); then
    ...
    echo
    echo "K15.4 QEMU qualification timed out after ${TEST_TIMEOUT}s." >&2
    kill -TERM "$qemu_pid" 2>/dev/null || true
    sleep 0.5
    kill -KILL "$qemu_pid" 2>/dev/null || true
fi
...
checker_status=0
"$ROOT/tools/check-k15-4-serial-log.sh" "$LOG" || checker_status=$?
