...
"$ROOT/tools/build.sh"
ESP="$ROOT/build/esp"
LOG="${K14C8_SERIAL_LOG:-$ROOT/build/k14c8-serial.log}"
NVME_IMAGE="${K14C8_NVME_IMAGE:-$ROOT/build/k14c8-nvme-test.img}"
NVME_SIZE="${K14C8_NVME_SIZE:-256M}"
...
mkdir -p "$ROOT/build"
OVMF_VARS_RUN="$ROOT/build/OVMF_VARS-K14C8.fd"
...
QEMU_ARGS=(
    ...
    -drive "if=none,id=k14c8nvme,format=raw,file=$NVME_IMAGE"
    -device nvme,drive=k14c8nvme,serial=TWK14C8NVME001
    ...
    -device edu,id=twk14c8iommutest,dma_mask=0xffffffffffffffff
    -monitor none
    -no-reboot
    -d guest_errors
)
...
cat <<MSG
Titanweave K14.C8 Radeon ASIC/IP identification staging QEMU test
  serial log     : $LOG
...
QEMU does not emulate a native Radeon. K14.C8 verifies the post-C6 Radeon ASIC/IP contract without promoting hardware:
...
checker_status=0
"$ROOT/tools/check-k14c8-serial-log.sh" "$LOG" || checker_status=$?
if (( status != 0 )); then
    exit "$status"
fi
exit "$checker_status"
