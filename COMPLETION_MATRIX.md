...
Qualified/frozen. Fedora/QEMU passed the operational GTT allocation/map/reclaim, VRAM reservation, GPU-VA reservation, AMD firmware parse/CRC/SHA staging path, watchdog/resource-safe software recovery, userspace handoff, `[QUAL]`, and intentional `[HALT]`. QEMU has no physical Radeon, so physical silicon upload/reset and C29 execution/DMA authority remain fenced.
