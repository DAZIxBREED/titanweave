...
impl RadeonMemoryManager{
 ...
 pub fn allocate_gtt(&mut self,allocator:&mut FrameAllocator<'_>,owner:u64,bytes:u64,alignment:u64)->Result<RadeonMemoryObject,&'static str>{
  ...
  // FrameAllocator returns page-aligned contiguous backing.  Larger alignment is
  // accepted only when the resulting physical allocation satisfies it.
...
fn real_gtt_self_test(allocator:&mut FrameAllocator<'_>,kernel_cr3:u64)->Result<(),&'static str>{
 ...
 // Page-table frames may have been allocated to establish the permanent DMA
 // aperture; require reclaim of the data pages, not equality with the old free count.
