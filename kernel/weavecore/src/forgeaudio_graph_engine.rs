...
//!
//! This gate adds the eight required operational graph nodes without taking
//! K15.9's sample-accurate switching scope. Graph topology is built and
//! compiled on the control path. The processing path is fixed-capacity,
//! allocation-free, lock-free in the sense that it takes no lock at all, and
//! performs no filesystem I/O or sleeping.
...
impl NodeKind {
    const fn execution_index(self) -> usize {
        self as usize - 1
    }
...
impl GraphState {
    ...
    fn add_node(&mut self, kind: NodeKind, param0: i32, param1: i32) -> Result<u8, &'static str> {
        if self.compiled {
            return Err("graph topology is immutable after compile");
        }
        if self.node_count >= MAX_GRAPH_NODES {
            return Err("graph node capacity exhausted");
        }
    ...
    fn compile(&mut self) -> Result<(), &'static str> {
        if self.compiled || self.node_count == 0 {
            return Err("graph compile state is invalid");
        }
                ...
                for slot in 0..self.nodes[target].kind.required_inputs() {
                    if self.nodes[target].inputs[slot] == node_index as u8 {
                        indegree[target] = indegree[target]
                            .checked_sub(1)
                            .ok_or("graph indegree underflow")?;
                    }
                }
            }
        }
    ...
    fn process_block(
        ...
        for order_index in 0..self.order_count {
            ...
            self.node_runs[node.kind.execution_index()] = self.node_runs[node.kind.execution_index()]
                .checked_add(1)
            ...
            match node.kind {
                NodeKind::Input => {
                    for sample in 0..GRAPH_SAMPLES {
                        let byte = sample * 2;
                        self.scratch[node_index][sample] =
                            i16::from_le_bytes([input[byte], input[byte + 1]]);
                    }
                }
                NodeKind::Gain => {
                    ...
                    for sample in 0..GRAPH_SAMPLES {
                        let scaled = (i32::from(source[sample]) * node.param0) >> 15;
                        self.scratch[node_index][sample] = clamp_i16(scaled);
                    }
                }
                NodeKind::Mixer => {
                    ...
                    for sample in 0..GRAPH_SAMPLES {
                        self.scratch[node_index][sample] = clamp_i16(
                            i32::from(source_a[sample]) + i32::from(source_b[sample]),
                        );
                    }
                }
                NodeKind::Splitter => {
                ...
                NodeKind::FormatConverter => {
                    // Exercise the frozen K15.5 layout semantics as an actual
                    // graph node: S16 stereo interleaved -> planar ->
                    // interleaved. The round trip is byte/sample exact.
                    let source = self.scratch[usize::from(node.inputs[0])];
                    let mut planar = [0i16; GRAPH_SAMPLES];
                    for frame in 0..GRAPH_FRAMES {
                        for channel in 0..GRAPH_CHANNELS {
                            planar[channel * GRAPH_FRAMES + frame] =
                                source[frame * GRAPH_CHANNELS + channel];
                        }
                    }
                    for frame in 0..GRAPH_FRAMES {
                        for channel in 0..GRAPH_CHANNELS {
                            self.scratch[node_index][frame * GRAPH_CHANNELS + channel] =
                                planar[channel * GRAPH_FRAMES + frame];
                        }
                    }
...
pub fn run_qualification() -> Result<GraphQualificationReport, &'static str> {
    ...
    let mapper = graph.add_node(NodeKind::ChannelMapper, 1, 0)?; // swap L/R
    ...
    for block in 0..4i16 {
        ...
        for sample in 0..GRAPH_SAMPLES {
            ...
            if actual != expected {
                return Err("graph output sample verification failed");
            }
        }
    }
