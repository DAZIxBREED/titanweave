...
pub struct ExecutionQueue{class:QueueClass,entries:[QueueEntry;C31_QUEUE_DEPTH],head:usize,tail:usize,count:usize,next:u64,submitted:u64,retired:u64,resets:u64,abandoned:u64}
impl ExecutionQueue{
 pub const fn new(class:QueueClass)->Self{Self{class,entries:[QueueEntry::EMPTY;C31_QUEUE_DEPTH],head:0,tail:0,count:0,next:1,submitted:0,retired:0,resets:0,abandoned:0}}
 ...
 pub fn reset(&mut self)->usize{let abandoned=self.count;self.entries=[QueueEntry::EMPTY;C31_QUEUE_DEPTH];self.head=0;self.tail=0;self.count=0;self.resets=self.resets.saturating_add(1);self.abandoned=self.abandoned.saturating_add(abandoned as u64);abandoned}
 pub fn stability_counters(&self)->(u64,u64){(self.resets,self.abandoned)}
}
pub fn queue_self_test()->Result<u64,&'static str>{let mut c=ExecutionQueue::new(QueueClass::Compute);let a=c.submit(11,1)?;if c.start_head()?!=a{return Err("C31 compute queue start failed")}if c.retire_head(1)?!=a{return Err("C31 compute queue retire failed")}let mut g=ExecutionQueue::new(QueueClass::Graphics);let b=g.submit(12,2)?;if g.start_head()?!=b||g.retire_head(2)?!=b{return Err("C31 graphics queue lifecycle failed")}let (_,cs,cr,cc)=c.counters();let(_,gs,gr,gc)=g.counters();if (cs,cr,cc,gs,gr,gc)!=(1,1,0,1,1,0){return Err("C31 queue counters failed")}let mut r=ExecutionQueue::new(QueueClass::Compute);let _=r.submit(13,3)?;let _=r.start_head()?;if r.reset()!=1||r.counters().3!=0||r.stability_counters()!=(1,1){return Err("C31/C32 queue reset lifecycle failed")}Ok(0xc031_5155_4555_0001^cs^(cr<<8)^(gs<<16)^(gr<<24))}
